...
response = await node.get_account_balances(TEST_ADDRESS)
```
...
```python [Python]
node = await NodeClient.connect(make_mainnet().node)
account = await node.get_account(ADDRESS)
```

```typescript [TypeScript]
const client = await ValidatorClient.connect(Network.mainnet().validatorConfig);
...
subaccount = await node.get_subaccount(TEST_ADDRESS, 0)
```
