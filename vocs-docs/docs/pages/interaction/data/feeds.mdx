...
Here, the update messages contain the finalized trades (fills) for the `ETH-USD` market/ticker.

For each received message, the `handler()` function will be called on it.
