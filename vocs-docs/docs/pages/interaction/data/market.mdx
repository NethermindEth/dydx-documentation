...
The current price is one of the most important parameters. To fetch all available markets and associated prices,

:::code-group

```python [Python]
prices = await node.get_prices() # [!code focus]
```

```typescript [TypeScript]
const prices = await node.get.getAllPrices(); // [!code focus]
```

```rust [Rust]
let prices = node.get_prices(None).await?; // [!code focus]
```
