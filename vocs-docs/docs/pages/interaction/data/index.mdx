...
Retrieve perpetual positions for a specific subaccount. This example below show you how to query and filter the active or historical perpetual positions associated with a particular subaccount.
...
subaccount = Subaccount.from_mnemonic(DYDX_TEST_MNEMONIC) # [!code focus]
address = subaccount.address # [!code focus]
perpetual_positions_response = client.account.get_subaccount_perpetual_positions(address, 0) # [!code focus]
```
