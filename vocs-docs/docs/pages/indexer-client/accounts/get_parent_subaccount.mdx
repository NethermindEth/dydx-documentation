### Get Parent Subaccount

Query for the parent subaccount, its child subaccounts, equity, collateral and margin.

#### Method Declaration

:::code-group

```rust [Rust]
pub async fn get_parent_subaccount(
    &self,
    subaccount: &ParentSubaccount,
) -> Result<ParentSubaccountResponseObject, Error>
```

```python [Python]

```

```typescript [TypeScript]
async getParentSubaccount(address: string, parentSubaccountNumber: number): Promise<Data>
```

```url [API]
/v4/addresses/{address}/parentSubaccountNumber/{number}
```

:::

:::details[Unification Plan]
:::

#### Parameters

| Parameter | Location  | Type                  | Description                                                           |
| --------- | --------- | --------------------- | --------------------------------------------------------------------- |
| `address` | path      | [Address]             | The wallet address that owns the parent subaccount.                   |
| `number`  | path      | [SubaccountNumber]    | The identifier for the specific subaccount within the wallet address. |


#### Response

| Status | Meaning     | Schema                          |
| ------ | ----------- | ------------------------------- |
| `200`  | [OK]        | [ParentSubaccountResponseObject] |

[Address]: /types/address
[u32]: /types/u32
[ParentSubaccountResponseObject]: /types/parent_subaccount_response_object